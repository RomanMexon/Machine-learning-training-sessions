...
class KNearestNeighbor:
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                s = np.sum((X[i] - self.X_train[j]) ** 2 ) ** 0.5
                dists[i, j] = s
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = (np.sum(X ** 2, axis=1)[:, None] + np.sum(self.X_train ** 2, axis=1)[None, :] - 2 * X @ self.X_train.T) ** 0.5
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            labels = np.argsort(current_dist)
            closest_y = self.y_train[labels[:k]]
